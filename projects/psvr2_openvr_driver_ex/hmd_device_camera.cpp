...
bool HmdDeviceCamera::GetCameraDistortion(uint32_t nCameraIndex, float flInputU, float flInputV, float *pflOutputU, float *pflOutputV) {
  ...
  if (nCameraIndex == 0) {
    DistortionParameters params{{4.410233544294857,
                                 8.984947311901681,
                                 -9.87816473467086e-7,
                                 0.0000074754945954835176,
                                 3.677508810670342,
                                 4.7174213801717,
                                 10.2783500115958,
                                 6.13988926548374,
                                 ...
                                 0,
                                 0,
                                 -0.0015530241474945375,
                                 -0.00039289988739191575,
                                 0.32970695313327514,
                                 0.005864799802305574,
                                 0.000009239037037645196,
                                 0.971584734607621,
                                 0.0367018715754216,
                                 0.000227653240934961}};

    applyDistortionTransform(out, &params, ((flInputU - 505.8196716308594f) / 379.56439208984375f) * k_ZoomFactor,
    ...
    *pflOutputV = ((out[1] * 379.56439208984375f) + 506.0033264160156f) / IMAGE_HEIGHT;
    ...
    DistortionParameters params{{4.408362227825522,
                                 8.984405837950234,
                                 0.0000027455310754732335,
                                 -0.0000025145806185925286,
                                 3.679019420961825,
                                 4.7174213801717,
                                 10.2783500115958,
                                 6.13988926548374,
                                 ...
                                 0,
                                 0,
                                 -0.0017732032029994756,
                                 0.00023450291427025443,
                                 0.3299951208112014,
                                 0.005861164619598151,
                                 0.00000932997375226993,
                                 0.971584734607621,
                                 0.0367018715754216,
                                 0.000227653240934961}};

    applyDistortionTransform(out, &params, ((flInputU - 503.70745849609375f) / 381.2378234863281f) * k_ZoomFactor,
...
bool HmdDeviceCamera::GetCameraProjection(uint32_t nCameraIndex, vr::EVRTrackedCameraFrameType eFrameType, float flZNear, float flZFar,
  ...
  const float fx = nCameraIndex == 0 ? 379.56439f : 381.23782f;
  const float fy = nCameraIndex == 0 ? 379.56439f : 381.23782f;
  const float cx = nCameraIndex == 0 ? 505.81967f : 503.70745f;
  const float cy = nCameraIndex == 0 ? 506.00332f : 504.45907f;
...
bool HmdDeviceCamera::GetCameraIntrinsics(uint32_t nCameraIndex, vr::EVRTrackedCameraFrameType eFrameType, vr::HmdVector2_t *pFocalLength,
  ...
  if (eFrameType == vr::VRTrackedCameraFrameType_Distorted) {
    ...
    if (rCoefficients) {
      for (int i = 0; i < vr::k_unMaxDistortionFunctionParameters; ++i)
        rCoefficients[i] = 0.0;
    }
  }
