    ...
    case Command_ClientRequestGazeData: {
      ...
      if (pHeader->dataLen == 0 && containsPort) {
        if (m_doGaze) {
          ...
          if (ipcVersion == 1) {
            CommandDataServerGazeDataResult_t response = {{leftData.isGazeOriginValid, leftData.gazeOriginMm, leftData.isGazeDirValid, leftData.gazeDirNorm,
                                                           leftData.isPupilDiaValid, leftData.pupilDiaMm, leftData.isBlinkValid, leftData.blink},
                                                          {rightData.isGazeOriginValid, rightData.gazeOriginMm, rightData.isGazeDirValid, rightData.gazeDirNorm,
                                                           rightData.isPupilDiaValid, rightData.pupilDiaMm, rightData.isBlinkValid, rightData.blink}};
            ...
            CommandDataServerGazeDataResult2_t response = {
                {leftData.isGazeOriginValid, leftData.gazeOriginMm, leftData.isGazeDirValid, leftData.gazeDirNorm, leftData.isPupilDiaValid,
                 leftData.pupilDiaMm, leftData.isBlinkValid, leftData.blink, leftData.isOpenEnabled, leftData.open},
                {rightData.isGazeOriginValid, rightData.gazeOriginMm, rightData.isGazeDirValid, rightData.gazeDirNorm, rightData.isPupilDiaValid,
                 rightData.pupilDiaMm, rightData.isBlinkValid, rightData.blink, rightData.isOpenEnabled, rightData.open}};
            SendIpcCommand(clientSocket, Command_ServerGazeDataResult, &response, sizeof(response));
          } else {
    ...
    case Command_ClientTriggerEffectMultiplePositionVibration: {
      ...
      if (containsPort) {
        if (pHeader->dataLen >= sizeof(EVRControllerType)) {
          ...
          case VRController_Left:
            mappedControllerType = VRControllerType::Left;
            break;
          case VRController_Right:
            mappedControllerType = VRControllerType::Right;
            break;
          case VRController_Both:
            mappedControllerType = VRControllerType::Both;
            break;
          }
          ...
          case Command_ClientTriggerEffectMultiplePositionFeedback: {
            ...
            if (pHeader->dataLen >= sizeof(CommandDataClientTriggerEffectMultiplePositionFeedback_t)) {
              CommandDataClientTriggerEffectMultiplePositionFeedback_t *req =
                  reinterpret_cast<CommandDataClientTriggerEffectMultiplePositionFeedback_t *>(pData);
          ...
          case Command_ClientTriggerEffectMultiplePositionVibration: {
            ...
            if (pHeader->dataLen >= sizeof(CommandDataClientTriggerEffectMultiplePositionVibration_t)) {
              CommandDataClientTriggerEffectMultiplePositionVibration_t *req =
                  reinterpret_cast<CommandDataClientTriggerEffectMultiplePositionVibration_t *>(pData);
