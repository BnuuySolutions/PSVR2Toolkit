...
int (*CaesarUsbThread::orig_report)(CaesarUsbThread *thisptr, uint8_t bIsSet, uint16_t reportId, void *buffer, uint16_t length, uint16_t value, uint16_t index,
                                    uint16_t subcmd) = nullptr;
...
int CaesarUsbThread::TransferPipe(uint8_t pipeId, char *buffer, size_t length, uint64_t timeoutMs) {
  ...
  if (isBulk) {
    libusb_fill_bulk_transfer(transfer, this->m_devHandle, pipeId, (unsigned char *)buffer, static_cast<int>(length), AsyncTransferCallback, &state, timeoutMs);
  } else {
    libusb_fill_interrupt_transfer(transfer, this->m_devHandle, pipeId, (unsigned char *)buffer, static_cast<int>(length), AsyncTransferCallback, &state,
                                   timeoutMs);
  }
...
void CaesarUsbThread::DestructorHook(CaesarUsbThread *thisptr, bool shouldFree) {
  ...
  if (orig_destructor)
    orig_destructor(thisptr, shouldFree);
}
...
int CaesarUsbThread::ControlCommandHook(CaesarUsbThread *thisptr, uint8_t bIsSet, uint16_t reportId, void *buffer, uint16_t length, uint16_t value,
                                        uint16_t index, uint16_t subcmd) {
  return thisptr->ControlCommand(bIsSet, reportId, buffer, length, value, index, subcmd);
}

int CaesarUsbThread::GetDescriptorHook(CaesarUsbThread *thisptr, libusb_device_descriptor *pDest) { return thisptr->GetDescriptor(pDest); }
...
void CaesarUsbThread::InstallHooks() {
  ...
  HookLib::InstallHook(reinterpret_cast<void *>(baseAddr + 0x1225a0), reinterpret_cast<void *>(DestructorHook), reinterpret_cast<void **>(&orig_destructor));

  HookLib::InstallHook(reinterpret_cast<void *>(baseAddr + 0x127b70), reinterpret_cast<void *>(ThreadLoopHook), reinterpret_cast<void **>(&orig_threadLoop));

  HookLib::InstallHook(reinterpret_cast<void *>(baseAddr + 0x16b540), reinterpret_cast<void *>(JoinThreadHook), reinterpret_cast<void **>(&orig_joinThread));

  HookLib::InstallHook(reinterpret_cast<void *>(baseAddr + 0x125bb0), reinterpret_cast<void *>(StartHook), reinterpret_cast<void **>(&orig_start));

  HookLib::InstallHook(reinterpret_cast<void *>(baseAddr + 0x124cd0), reinterpret_cast<void *>(GetInterfaceHook),
                       reinterpret_cast<void **>(&orig_getInterface));

  HookLib::InstallHook(reinterpret_cast<void *>(baseAddr + 0x124f60), reinterpret_cast<void *>(GetEndpointHook), reinterpret_cast<void **>(&orig_getEndpoint));

  HookLib::InstallHook(reinterpret_cast<void *>(baseAddr + 0x125660), reinterpret_cast<void *>(OnConnectedHook), reinterpret_cast<void **>(&orig_onConnected));

  HookLib::InstallHook(reinterpret_cast<void *>(baseAddr + 0x125b40), reinterpret_cast<void *>(OnDisconnectHook),
                       reinterpret_cast<void **>(&orig_onDisconnect));

  HookLib::InstallHook(reinterpret_cast<void *>(baseAddr + 0x125cb0), reinterpret_cast<void *>(PollAndProcessHook),
                       reinterpret_cast<void **>(&orig_pollAndProcess));

  HookLib::InstallHook(reinterpret_cast<void *>(baseAddr + 0x127d60), reinterpret_cast<void *>(ReadPipeHook), reinterpret_cast<void **>(&orig_read));

  HookLib::InstallHook(reinterpret_cast<void *>(baseAddr + 0x1283f0), reinterpret_cast<void *>(ControlCommandHook), reinterpret_cast<void **>(&orig_report));

  HookLib::InstallHook(reinterpret_cast<void *>(baseAddr + 0x124510), reinterpret_cast<void *>(GetDescriptorHook),
