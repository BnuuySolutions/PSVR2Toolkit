...
private:
  ...
  struct OverlappedWithData {
    OverlappedWithData(AsyncFileWriter *asyncWriter) : overlapped(), writerInstance(asyncWriter), hFile(NULL), hTimer(NULL), timeoutCallback(nullptr) {
    ...
    AsyncFileWriter *writerInstance;
    HANDLE hFile;
    HANDLE hTimer;
...
public:
  AsyncFileWriter(size_t initialPoolSize = 16) : m_pool(this, initialPoolSize) {}
  ...
  long GetPendingOperationCount() { return pendingOperations; }
...
private:
  ...
  static VOID CALLBACK FileWriteCompletionRoutine(DWORD dwErrorCode, DWORD dwNumberOfBytesTransfered, LPOVERLAPPED lpOverlapped) {
