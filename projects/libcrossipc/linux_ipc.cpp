...
LinuxIpcMutex::LinuxIpcMutex(const char* name) : m_name(name), m_fd(-1), m_mutex(nullptr) {
    ...
    if (m_fd != -1) {
        initialize = true;
        if (ftruncate(m_fd, sizeof(pthread_mutex_t)) == -1) throw std::runtime_error("ftruncate failed");
    } else {
        m_fd = shm_open(shmName.c_str(), O_RDWR, 0666);
        if (m_fd == -1) throw std::runtime_error("shm_open failed");
        struct stat st;
        while (true) {
            fstat(m_fd, &st);
            if (st.st_size >= (off_t)sizeof(pthread_mutex_t)) break;
            usleep(1000);
        }
    }
...
LinuxIpcMutex::~LinuxIpcMutex() {
    if (m_mutex && m_mutex != MAP_FAILED) munmap(m_mutex, sizeof(pthread_mutex_t));
...
LinuxIpcEvent::LinuxIpcEvent(const char* name) : m_name(name), m_fd(-1), m_data(nullptr) {
    std::string shmName = "/" + m_name + "_EVT_SHM";
    m_fd = shm_open(shmName.c_str(), O_CREAT | O_EXCL | O_RDWR, 0666);
    bool initialize = false;
    if (m_fd != -1) {
        initialize = true;
        if (ftruncate(m_fd, sizeof(EventData)) == -1) throw std::runtime_error("ftruncate failed");
    } else {
        m_fd = shm_open(shmName.c_str(), O_RDWR, 0666);
        if (m_fd == -1) throw std::runtime_error("shm_open failed");
        struct stat st;
        while (true) {
            fstat(m_fd, &st);
            if (st.st_size >= (off_t)sizeof(EventData)) break;
            usleep(1000);
        }
    }
...
LinuxIpcEvent::~LinuxIpcEvent() {
    if (m_data && m_data != MAP_FAILED) munmap(m_data, sizeof(EventData));
    if (m_fd != -1) close(m_fd);
}

void LinuxIpcEvent::set() {
    int ret = pthread_mutex_lock(&m_data->mutex);
    if (ret == EOWNERDEAD) pthread_mutex_consistent(&m_data->mutex);
    ...
    if (!m_data->signaled) {
        if (timeoutMs == 0xFFFFFFFF) {
            ...
            while (!m_data->signaled) {
                ...
                if (err == ETIMEDOUT) {
                    if (!m_data->signaled) success = false;
                    break;
                }
            }
        }
...
LinuxIpcSharedMemory::LinuxIpcSharedMemory(const char *name, size_t size) : m_name(name), m_fd(-1), m_size(size), m_ptr(nullptr) {
    ...
    if (st.st_size < (off_t)size) {
        if (ftruncate(m_fd, size) == -1) throw std::runtime_error("ftruncate failed");
    }
}
...
LinuxIpcBroadcast::LinuxIpcBroadcast(const char *name) : m_name(name), m_fd(-1), m_data(nullptr), m_last_val(0) {
    std::string shmName = "/" + m_name + "_BCAST_SHM";
    m_fd = shm_open(shmName.c_str(), O_CREAT | O_EXCL | O_RDWR, 0666);
    bool initialize = false;
    if (m_fd != -1) {
        initialize = true;
        if (ftruncate(m_fd, sizeof(BroadcastData)) == -1) throw std::runtime_error("ftruncate failed");
    } else {
        m_fd = shm_open(shmName.c_str(), O_RDWR, 0666);
        if (m_fd == -1) throw std::runtime_error("shm_open failed");
        struct stat st;
        while (true) {
            fstat(m_fd, &st);
            if (st.st_size >= (off_t)sizeof(BroadcastData)) break;
            usleep(1000);
        }
    }
...
LinuxIpcBroadcast::~LinuxIpcBroadcast() {
    if (m_data && m_data != MAP_FAILED) munmap(m_data, sizeof(BroadcastData));
    if (m_fd != -1) close(m_fd);
}
