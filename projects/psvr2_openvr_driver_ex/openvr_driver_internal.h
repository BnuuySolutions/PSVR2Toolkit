...
class IVRBlockQueue {
public:
  virtual EBlockQueueError Create(PropertyContainerHandle_t *pulQueueHandle, const char *pchPath, uint32_t unBlockDataSize, uint32_t unBlockHeaderSize,
                                  uint32_t unBlockCount, uint32_t unFlags = 0) = 0;
  ...
  virtual EBlockQueueError WaitAndAcquireReadOnlyBlock(PropertyContainerHandle_t ulQueueHandle, PropertyContainerHandle_t *pulBlockHandle, void **ppvBuffer,
                                                       EBlockQueueReadType eReadType, uint32_t unTimeoutMs) = 0;
  virtual EBlockQueueError AcquireReadOnlyBlock(PropertyContainerHandle_t ulQueueHandle, PropertyContainerHandle_t *pulBlockHandle, void **ppvBuffer,
...
template <typename T> struct VrPathTagTraits;

template <> struct VrPathTagTraits<uint64_t> {
  static constexpr vr::PropertyTypeTag_t tag = vr::k_unUint64PropertyTag;
};
template <> struct VrPathTagTraits<int32_t> {
  static constexpr vr::PropertyTypeTag_t tag = vr::k_unInt32PropertyTag;
};
template <> struct VrPathTagTraits<double> {
  static constexpr vr::PropertyTypeTag_t tag = vr::k_unDoublePropertyTag;
};
template <> struct VrPathTagTraits<float> {
  static constexpr vr::PropertyTypeTag_t tag = vr::k_unFloatPropertyTag;
};
template <> struct VrPathTagTraits<bool> {
  static constexpr vr::PropertyTypeTag_t tag = vr::k_unBoolPropertyTag;
};
 ...
 */
template <typename T> void WritePathProperty(vr::IVRPaths *pVRPaths, vr::PropertyContainerHandle_t hContainer, const char *pchPath, T value) {
  if (!pVRPaths || hContainer == vr::k_ulInvalidPropertyContainer)
    return;
